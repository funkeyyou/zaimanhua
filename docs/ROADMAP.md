...
## v2.1.0：閱讀器與導航整理（2026-09-24）

- 關於頁作者資訊更新為 funkeyyou。
- 閱讀器與導航的內部調整及穩定性改善。
- 104 項測試及雙平台 CI 通過，已正式發布；見 [發版紀錄](QA-v2.1.0.md)。

## v2.0.1：跨集翻頁改善（2026-09-11）

- 橫向跨集移除圓形箭頭動畫與拉動刷新門檻，支援短距離快滑。
- 保留縮放與上下滑動防誤觸，避免同一手勢滑到章節末尾後直接跳集。
- Android 使用者驗收通過，81 項測試及雙平台 CI 通過，已正式發布；見 [發版紀錄](QA-v2.0.1.md)。

## v2.0.0：閱讀入口與介面整理（2026-09-09）

- 書架新增自適應最近閱讀列；訂閱列表保持預設更新時間降序，允許同一本作品在兩區同時出現。
- 紀錄入口移至書架右上角，預設本機紀錄並提供雲端入口；修正本機紀錄頁誤用雲端 View。
- 移除「我的」重複入口，以常用功能、帳號福利、偏好與關於分組；詳情頁整理閱讀主按鈕及章節進度。
- 修正橫向主頁固定 450 寬；無詳情時使用全寬，寬度足夠且開啟詳情時再分欄。
- 顯示名稱採用使用者指定的「再漫畫X」；新書頁 X 圖示同步 Android adaptive／單色圖示及 Windows ICO。
- 閱讀進度改為單一下拉選單，與標籤、狀態、排序同列；漫畫／小說首頁輪播限制高度並顯示相鄰推薦。
- 修正簡介展開反而截短，以及更多章節按鈕大小和樣式不一致。
- 78 個測試與雙平台 CI 通過，MuMu 及 Windows 主流程已驗證；測試包位於 [CI 34275340195](https://github.com/funkeyyou/zaimanhua/actions/runs/34275340195)，完整驗證及限制見 [UI QA](QA-2026-09-09-UI.md)。
...
- v2.1.0（2026-09-24，已正式發佈）：關於頁作者資訊更新、閱讀器與導航內部整理。104 項測試與正式 Android／Windows 建置通過，APK 與前版同簽章。見 [QA-v2.1.0](QA-v2.1.0.md)。
- v2.0.0（2026-09-09，已正式發佈）：名稱改為再漫畫X，保留已驗證的書頁 X 圖示；整合最近閱讀與書架入口、精簡閱讀進度篩選、重新分組「我的」、強化詳情閱讀操作、大螢幕全寬／並排與輪播適配，修正簡介展開及更多章節按鈕。78 個測試與正式 Android／Windows 建置通過，APK 同簽章，更新 API 與下載連結正常；下載頁僅保留最新版。見 [QA-v2.0.0](QA-v2.0.0.md)。
