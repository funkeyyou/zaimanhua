...
[最新版 v2.0.1](../../releases/latest) 提供安卓 APK 與 Windows 免安裝壓縮檔。App 內「我的 → 檢查更新」讀的也是同一處。

v2.0.1 改善漫畫橫向跨集手勢，最後一頁短距離快滑即可進入下一集，移除圓形箭頭動畫與額外長拉操作。既有書架、紀錄和設定可繼續使用，Android 可直接覆蓋安裝。完整內容見 [版本說明](docs/releases/v2.0.1.md)，發版檢查見 [驗證紀錄](docs/QA-v2.0.1.md)。
